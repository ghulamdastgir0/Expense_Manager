...
function Reports() {
  ...
  const fetchReports = useCallback(async (period) => {
    ...
    try {
      ...
      // apiFetch returns the raw parsed JSON body, so incomeRes IS the full response.
      // Backend may return either:
      //   Shape A: { success: true, data: { income: X, expenses: Y } }
      //   Shape B: { success: true, income: X, expenses: Y }          (flat)
      //   Shape C: { success: true, data: { total_income: X, total_expenses: Y } }
      // We try all of them.
      console.log("RAW incomeRes (full):", JSON.stringify(incomeRes));

      const rawIncome =
        (incomeRes?.data && typeof incomeRes.data === "object" && !Array.isArray(incomeRes.data))
          ? incomeRes.data   // Shape A or C — unwrap .data
          : incomeRes || {}; // Shape B — flat, use as-is

      const incomeVsExpense = {
        income: parseFloat(
          rawIncome.income ||
          rawIncome.total_income ||
          rawIncome.totalIncome ||
          0
        ),
        expenses: parseFloat(
          rawIncome.expenses ||
          rawIncome.total_expenses ||
          rawIncome.totalExpenses ||
          0
        )
      };

      console.log("rawIncome resolved:", JSON.stringify(rawIncome));
      console.log("incomeVsExpense extracted:", incomeVsExpense);

      // FALLBACK: if incomeVsExpense.income is still 0, sum it from the
      // monthly_comparison data (same source that powers the bar chart correctly).
      // This handles the case where /reports/income-vs-expense backend endpoint
      // silently excludes income transactions due to a backend bug.
      const monthly = monthlyRes?.data?.monthly_comparison || [];
      const fallbackIncome   = monthly.reduce((s, m) => s + parseFloat(m.income   || 0), 0);
      const fallbackExpenses = monthly.reduce((s, m) => s + parseFloat(m.expenses || 0), 0);

      const finalIncomeVsExpense = {
        income:   incomeVsExpense.income   > 0 ? incomeVsExpense.income   : fallbackIncome,
        expenses: incomeVsExpense.expenses > 0 ? incomeVsExpense.expenses : fallbackExpenses,
      };
      console.log("finalIncomeVsExpense (after fallback):", finalIncomeVsExpense);

      setReportData({
        monthly,
        topCategories: topRes?.data?.top_categories || [],
        paymentModes: paymentRes?.data?.payment_modes || [],
        incomeVsExpense: finalIncomeVsExpense
      });
    } catch (err) {
  ...
  }, [timePeriod, fetchReports]);

  const filteredData = useMemo(() => {
    const slots = buildMonthSlots();
    const dataMap = Object.fromEntries(
      reportData.monthly.map((m) => [m.month_key, m])
    );
    const mergedMonthly = slots.map((slot) =>
      dataMap[slot.month_key]
        ? {
            ...slot,
            income: dataMap[slot.month_key].income,
            expenses: dataMap[slot.month_key].expenses
          }
        : slot
    );
    ...
    const filteredCategories =
      selectedCategory === "all"
        ...
        : reportData.topCategories.filter(
            (c) => c.category === selectedCategory
          );

    const expenseData = filteredCategories.map((c) => ({
      category: c.category,
      expenses: c.total_spent
    }));
    const categoryData = filteredCategories.map((c) => ({
      name: c.category,
      value: c.total_spent
    }));
    ...
    return (
      <div className="p-6 text-white bg-black min-h-screen flex items-center justify-center">
        <div className="text-center space-y-3">
          <svg
            className="animate-spin h-8 w-8 mx-auto text-[#4ADE80]"
            viewBox="0 0 24 24"
            fill="none"
          >
            <circle
    ...
    <div className="p-6 bg-black text-white min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-gray-900 border border-gray-700 rounded-xl p-4">
          ...
          <p
            className={`text-2xl font-bold ${net >= 0 ? "text-blue-400" : "text-red-400"}`}
          >
            {net < 0 ? "-" : ""}
